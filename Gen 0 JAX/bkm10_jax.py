...
class BKM10():
    ...
    def __init__(self, k, Q2, x, t, FIXED_CFFS={}, helc=0, twist=2):
        '''
        ...
        self.ALPHA = 1/137.0359998
        self.M2 = 0.93827208816*0.93827208816
        self.GeV_to_nb = 0.389379e6

        # This section creates a mask array with elements 1 if CFF value is provided and 0 if not
        # and a fixed_vals array with elements 0 if value is not provided and the value itself otherwise
        ...
        self.mask = jnp.asarray(mask_vector)
        self.fixed_vals = jnp.asarray(fixed_vector)
        ...
        # BKM 2002 : Eq (31)
        self.t_min = -Q2*(2.*(1.-x)*(1.-jnp.sqrt(1+e2))+e2)/(4.*x*(1.-x)+e2)
        t_min = self.t_min

        # BKM 2010 : EQ (2.5) and BKM 2002 : EQ (30)?
        self.K_tilde = jnp.sqrt((t_min-t)*((1.-x)*jnp.sqrt(1.+e2) + (t-t_min)*(e2+4.*x*(1.-x))/(4.*Q2)) *
                               (1.-y-y*y*e2/4.)/(1.-y+y*y*e2/4.))
        K_tilde = self.K_tilde

        # self.K_tilde = jnp.sqrt((t_min-t)*((1.-x)*jnp.sqrt(1.+e2) + (t_min-t)*(e2+4.*x*(1.-x))/(4.*Q2)))

        # BKM 2010 : Below Eq (2.21)
        self.K = jnp.sqrt((1.-y+e2*y*y*0.25)/Q2)*K_tilde

        self.F1, self.F2 = self.form_factors(t)

        # BKM 2010 : prefactor in Eq (1.1)
        self.dsig_prefactor = self.ALPHA*self.ALPHA*self.ALPHA * \
            x*y*y/(8.*self.PI*Q2*Q2*jnp.sqrt(1.+e2))
    ...
    def calculate_P1_P2(self, phi):
        """
        Computes P1 and P2 terms from BKM 2002 paper
        """
        # BKM 2002 : Eq (29)
        kDelta = -(self.Q2/(2.*self.y*(1.+self.e2))) * (1. + 2.*self.K*jnp.cos(phi) -
    ...
    def calculate_BH_DVCS_I(self, phi, cffs):
        """
        Calculates intermediate amplitudes of BH, DVCS and Interference which contribute to the cross section
        """

        P1, P2 = self.calculate_P1_P2(phi)
    ...
    def calculate_cross_section(self, phi, cffs):
        ...
        T_BH_2, T_DVCS_2, I = self.calculate_BH_DVCS_I(phi, cffs)
    ...
    def compute_BH_amplitude(self, phi, P1, P2):
        ...
        TBH2 = 1./(self.x*self.x*self.y*self.y*(1.+self.e2)*(1.+self.e2)*self.t*P1*P2) * \
            (c0_BH + c1_BH*jnp.cos(phi) + c2_BH *
             jnp.cos(2.*phi) + s1_BH*jnp.sin(phi))
    ...
    def compute_DVCS_amplitude(self, phi, ReH, ImH, ReHt, ImHt, ReE, ImE, ReEt, ImEt):
        ...
        TDVCS2 = 1./(self.y*self.y*self.Q2) * (c0_DVCS + c1_DVCS *
                                               jnp.cos(phi) + s1_DVCS*jnp.sin(phi))
    ...
    def compute_Interference(self, phi, P1, P2, ReH, ImH, ReHt, ImHt, ReE, ImE):
        ...
        I = 1./(self.x*self.y*self.y*self.y*self.t*P1*P2) * \
            (c0_I + c1_I*jnp.cos(phi) + c2_I*jnp.cos(2.*phi) +
             c3_I*jnp.cos(3.*phi) + s1_I*jnp.sin(phi) + s2_I*jnp.sin(2.*phi))
    ...
    def form_factors(self, t):
        """
        Computes the Dirac (F1) and Pauli (F2) proton form factors using the dipole approximation for Sachs form factors (dimensionless).
        ...
        # in that paper, GE and GM are functions of Q^2. But Q^2 there is the nucleon momentum transfer, which is equal to -t in the DVCS process.
    ...
    def DVCS_UP_coeffs(self, ReH, ImH, ReHt, ImHt, ReE, ImE, ReEt, ImEt):
        ...
        s1_A = 8.*K*(-self.helc*y*jnp.sqrt(1.+e2))*Im_CC1/((2.-x)*(1.+e2))
    ...
    def coeffs_plus_plus_UP(self, ReCC, ReCC_V, ReCC_A, ImCC, ImCC_V, ImCC_A):
        ...
        s2_pp = -helc * (4.*A*y*(1.+RE-2.*x)*TPQ/(RE**3.)) * \
        ...
        SS1_pp = jnp.where(s1_pp!=0.0,
                          ImCC + s1_pp_V*ImCC_V/s1_pp + s1_pp_A*ImCC_A/s1_pp,
                          ImCC)
        SS2_pp = jnp.where(s2_pp!=0.0,
    ...
    def coeffs_zero_plus_UP(self, ReCC_eff, ReCC_V_eff, ReCC_A_eff, ImCC_eff, ImCC_V_eff, ImCC_A_eff):
        ...
        c0_0p = (12.*jnp.sqrt(2.*A)*K*(2.-y)/RE5) * (e2+(2.-6.*x-e2)*TQ/3.)

        c1_0p = (8.*jnp.sqrt(2.*A)/EE) * \
            ((2.-y)*(2.-y)*TPQ*(1.-x+((1.-x)*x+0.25*e2)*TPQ/RE) +
             (A/RE)*(1.-(1.-2.*x)*TQ)*(e2-2.*(1.+0.5*e2/x)*x*TQ))

        c2_0p = -(8.*jnp.sqrt(2.*A)*K*(2.-y)*(1.+0.5*e2)/RE5) * \
            (1. + (1. + 0.5*e2/x)*x*TQ/(1.+0.5*e2))

        s1_0p = helc*8.*jnp.sqrt(2.*A)*(2.-y)*y*KTQ/EE

        s2_0p = helc*(8.*jnp.sqrt(2.*A)*K*y*(1.+0.5*e2)/EE) * \
            (1. + (1.+0.5*e2/x)*x*TQ/(1.+0.5*e2))

        c0_0p_V = (24.*jnp.sqrt(2.*A)*K*(2.-y)*x*TQ/RE5)*(1.-(1.-2.*x)*TQ)

        c1_0p_V = (16.*jnp.sqrt(2.*A)*x*TQ/RE5) * \
            (KTQ*(2.-y)*(2.-y) + (1.-(1.-2.*x)*TQ)*(1.-(1.-2.*x)*TQ)*A)

        c2_0p_V = c0_0p_V/3.

        s1_0p_V = helc*(4.*jnp.sqrt(2.*A)*y*(2.-y)*x*TQ/EE) * \
            (4.*(1.-2.*x)*TQ*(1.+x*TQ)+e2*(1.+TQ)*(1.+TQ))

        s2_0p_V = -helc*(8.*jnp.sqrt(2.*A)*K*y*x*TQ/EE)*(1.-(1.-2.*x)*TQ)

        c0_0p_A = (4.*jnp.sqrt(2.*A)*K*(2.-y)*TQ*(8.-6.*x+5.*e2)/RE5) * \
            (1.-TQ*(2.-12.*x*(1.-x)-e2)/(8.-6.*x+5.*e2))

        c1_0p_A = (8.*jnp.sqrt(2.*A)*TQ/RE5) * \
            (KTQ*(1.-2.*x)*(2.-y)*(2.-y)+(1.-(1.-2.*x)*TQ)
             * A*(4.-2.*x+3.*e2+TQ*(4.*x*(1.-x)+e2)))

        c2_0p_A = (8.*jnp.sqrt(2.*A)*K*(2.-y)*TQ/EE) * \
            (1.-x+0.5*TPQ*(4.*x*(1.-x)+e2)/RE)

        s1_0p_A = -helc*8.*jnp.sqrt(2.*A)*y*(2.-y)*(1.-2.*x)*TQ*K*K/EE

        s2_0p_A = -helc*(2.*jnp.sqrt(2.*A)*K*y*TQ/EE) * \
            (4.-4.*x+2.*e2+4.*TQ*(4.*x*(1.-x)+e2))

        # BKM 2010 : Eq (2.37)

        B = jnp.sqrt(2.)*KTQ/(2.-x)
    ...
    def coeffs_minus_plus_UP(self, ReCC_T, ReCC_V_T, ReCC_A_T, ImCC_T, ImCC_V_T, ImCC_A_T):
        ...
        c1_mp = ((8.*K/(RE**3.)))*((2.-y)*(2.-y)*((2.-RE)/(1.+e2)) *
        ...
        c0_mp_A = (4.*(2.-y)*TQ/EE)*(TPQ*A*(2.*x*x-e2-3.*x+x*RE) +
                                     (KTQ/RE)*(4.-2.*x*(2.-y)*(2.-y)-4.*y+y*y-(RE**3.)))
        ...
        c2_mp_A = (16.*(2.-y)*A*TQ/(RE**3.))*(KTQ*(1.-2.*x)/(1.+e2)-(1.-x)*(2.*x*x-e2-3.*x-x*RE)/(4.*x*(1.-x)+e2) -
