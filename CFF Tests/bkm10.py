...
class BKM10(keras.layers.Layer):
    ...
    def calculate_cross_section(self, phi, cffs):
        ...
        phi = tf.expand_dims(phi, axis=0) # (1, number of phi points) for CFF batching
        ...
        ReH  = tf.expand_dims(ReH, axis=1)  # (N, 1) for CFF batching
        ReHt = tf.expand_dims(ReHt, axis=1)
        ReE  = tf.expand_dims(ReE, axis=1)
        ReEt = tf.expand_dims(ReEt, axis=1)
        ImH  = tf.expand_dims(ImH, axis=1)
        ImHt = tf.expand_dims(ImHt, axis=1)
        ImE  = tf.expand_dims(ImE, axis=1)
        ImEt = tf.expand_dims(ImEt, axis=1)
